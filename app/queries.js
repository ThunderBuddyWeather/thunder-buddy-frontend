...
export const usePushUser = (BASE_URL, authToken) => {
  ...
  return useMutation(
    (payload) => pushUser({ payload, BASE_URL, authToken }),
    {
      onSuccess: () => {
...
export const fetchFriendRequests = async ({ queryKey }) => {
  ...
  export const useFriendRequests = (username, BASE_URL, authToken) => {
    return useQuery(['friendRequests', { username, BASE_URL, authToken }], fetchFriendRequests, {
  ...
  export const acceptFriendRequest = async ({ senderUsername, friendUsername, BASE_URL, authToken }) => {
    const response = await fetch(`${BASE_URL}/api/friendship/accept/${senderUsername}+${friendUsername}`, {
      method: 'PUT',
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${authToken}`,
  ...
  export const rejectFriendRequest = async ({ senderUsername, friendUsername, BASE_URL, authToken }) => {
    const response = await fetch(`${BASE_URL}/api/friendship/reject/${senderUsername}+${friendUsername}`, {
      method: 'DELETE',
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${authToken}`,
