...
export default function AlertCard() {
  ...
  const { weatherCoords, authToken, user, BASE_URL, expoPushToken } =
    useAppContext();
    ...
    alerts: [
      {
        ...
        severity: 'Advisory',
        title:
          'Flood Advisory issued August 22 at 4:19PM EDT until August 22 at 6:15PM EDT by NWS Melbourne FL',
  ...
  useEffect(() => {
    if (weatherCoords && weatherCoords.latitude && weatherCoords.longitude) {
      fetch(
        `${BASE_URL}/api/weather-alerts?lat=${weatherCoords.latitude}&lon=${weatherCoords.longitude}`
      )
        .then(response => {
          if (!response.ok) {
    ...
    <View style={{ margin: 16, width: '100%' }}>
      ...
      <Portal>
        ...
        >
          ...
          <Text style={{ fontSize: 16, marginBottom: 4 }}>
            <Text style={{ fontWeight: 'bold' }}>Effective:</Text>{' '}
          ...
          <Text style={{ fontSize: 16, marginBottom: 4 }}>
            <Text style={{ fontWeight: 'bold' }}>Affected Regions:</Text>{' '}
            {alert?.regions ? alert.regions.join(', ') : ''}
          </Text>
          <ScrollView
            style={{
              backgroundColor: '#eee',
